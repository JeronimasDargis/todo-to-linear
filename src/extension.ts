...
async function createIssue(parsedText: String) {
	const teams = await client1.teams();
	const projects = await client1.projects();
	const selectedProject = projects.nodes.find((project) => project.slugId === projectId) || {id: ''};
	const RCCO_TEAM = teams.nodes[0];

  if (RCCO_TEAM.id) {
	await client1.createIssue({ projectId: selectedProject.id ,teamId: RCCO_TEAM.id, title: `[TODO]: ${parsedText}` });
  }
  }
...
export function activate(context: vscode.ExtensionContext) {
	...
	vscode.languages.registerHoverProvider(
		...
		new (class implements vscode.HoverProvider {
		  provideHover(
			...
			const commentCommandUri = vscode.Uri.parse(`command:todo-to-linear.createTask`);
			const contents = new vscode.MarkdownString(`[Create Todo](${commentCommandUri})`);
	...
	let hoverListener = vscode.commands.registerCommand('todo-to-linear.createTask', () => {
